...
const AddRecipe = () => {
    ...
    return (
        <form onSubmit={handleSubmit} className="max-w-4xl mx-auto p-4 sm:p-6 pb-32 animate-in fade-in">
            ...
            <div className="sticky top-16 pt-4 pb-4 mb-6 sm:mb-8 z-60 bg-white/95 backdrop-blur-md border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                ...
                <button type="submit" disabled={saving} className="w-full sm:w-auto bg-gray-900 text-white px-8 py-3 sm:py-3.5 rounded-xl font-black hover:bg-orange-600 transition-all shadow-lg shadow-gray-200 active:scale-95 disabled:opacity-50">
            ...
            <div className="space-y-6 sm:space-y-10">
                {/* Basic Information */}
                <section className="bg-gray-50 p-5 sm:p-8 rounded-3xl border-2 border-gray-100 space-y-6">
                    <h2 className="text-lg sm:text-xl font-black text-gray-800 border-b-2 border-gray-200 pb-2">1. Basics</h2>

                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-2">Recipe Name *</label>
                        <input required type="text" name="name" value={formData.name} onChange={handleChange} placeholder="e.g., Grandma's Lasagna" className="w-full p-3.5 sm:p-4 rounded-xl border-2 border-gray-200 focus:border-orange-500 outline-none" />
                    </div>
                    ...
                    <div>
                        ...
                        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6">
                            <div className="w-full sm:w-32 h-40 sm:h-32 shrink-0 rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50 flex flex-col items-center justify-center overflow-hidden relative group">
                                {formData.imageUrl ? (
                                    <>
                            ...
                            <div className="flex-1 w-full text-center sm:text-left">
                                ...
                                <p className="text-xs font-medium text-gray-400 mt-3">
                                    Supports JPG, PNG, or WEBP. You can also press <kbd className="bg-gray-200 px-1 rounded text-gray-700 hidden sm:inline">Ctrl+V</kbd> <span className="sm:hidden">paste</span> to paste an image.
                                </p>
                    ...
                    <div>
                        ...
                        <textarea name="summary" value={formData.summary} onChange={handleChange} placeholder="A brief description of this dish..." rows={3} className="w-full p-3.5 sm:p-4 rounded-xl border-2 border-gray-200 focus:border-orange-500 outline-none resize-y" />
                    </div>
                </section>

                {/* Classification */}
                <section className="bg-gray-50 p-5 sm:p-8 rounded-3xl border-2 border-gray-100 space-y-6">
                    <h2 className="text-lg sm:text-xl font-black text-gray-800 border-b-2 border-gray-200 pb-2">2. Classification</h2>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        <div>
                            <label className="block text-sm font-bold text-gray-700 mb-2">Category *</label>
                            <select required name="categoryId" value={formData.categoryId} onChange={handleChange} className="w-full p-3.5 sm:p-4 rounded-xl border-2 border-gray-200 focus:border-orange-500 outline-none bg-white">
                        ...
                        <div>
                            ...
                            <select name="subcategoryId" value={formData.subcategoryId} onChange={handleChange} disabled={!formData.categoryId} className="w-full p-3.5 sm:p-4 rounded-xl border-2 border-gray-200 focus:border-orange-500 outline-none bg-white disabled:bg-gray-100">
                ...
                {/* Timing & Yield */}
                <section className="bg-orange-50/50 p-5 sm:p-8 rounded-3xl border-2 border-orange-100 space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
                        <div>
                            ...
                            <input type="number" name="prepTime" value={formData.prepTime} onChange={handleChange} min="0" className="w-full p-3.5 sm:p-4 rounded-xl border-2 border-orange-100 focus:border-orange-500 outline-none bg-white" />
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-orange-800 mb-2">Cook Time (min)</label>
                            <input type="number" name="cookTime" value={formData.cookTime} onChange={handleChange} min="0" className="w-full p-3.5 sm:p-4 rounded-xl border-2 border-orange-100 focus:border-orange-500 outline-none bg-white" />
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-orange-800 mb-2">Servings</label>
                            <input type="number" name="servings" value={formData.servings} onChange={handleChange} min="1" className="w-full p-3.5 sm:p-4 rounded-xl border-2 border-orange-100 focus:border-orange-500 outline-none bg-white" />
                        </div>
                    </div>
                </section>

                {/* Ingredients (DND Implementation) */}
                <section className="bg-gray-50 p-3 sm:p-8 rounded-3xl border-2 border-gray-100 space-y-4 sm:space-y-6">
                    <div className="flex justify-between items-end border-b-2 border-gray-200 pb-2 px-2 sm:px-0">
                ...
                {/* Instructions & Notes */}
                <section className="bg-gray-50 p-5 sm:p-8 rounded-3xl border-2 border-gray-100 space-y-6">
                    <h2 className="text-lg sm:text-xl font-black text-gray-800 border-b-2 border-gray-200 pb-2">5. Directions</h2>

                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-2">Instructions * <span className="text-gray-400 font-normal block sm:inline">(Separate steps with a new line)</span></label>
                        <textarea required name="instructions" value={formData.instructions} onChange={handleChange} rows={8} className="w-full p-4 sm:p-5 rounded-2xl border-2 border-gray-200 focus:border-orange-500 outline-none resize-y leading-relaxed text-sm sm:text-base" placeholder="1. Preheat the oven...&#10;2. Mix the dry ingredients..." />
                    </div>
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            rows={4}
                            className="w-full p-4 sm:p-5 rounded-2xl border-2 border-gray-200 focus:border-orange-500 outline-none resize-y bg-yellow-50/30 focus:bg-white transition-colors text-sm sm:text-base"
