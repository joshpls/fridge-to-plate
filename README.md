...
The project follows a Controller-Service-Repository pattern to ensure a clean separation of concerns:
    ...
    Routes: Define endpoints and apply middleware (e.g., requireAuth).

    Controllers: Handle HTTP requests/responses and extract parameters.

    Services: Contain the "business logic" (e.g., the matching algorithm for pantry items).

    Prisma/DB: Interacts directly with the PostgreSQL instance.

🔒 Security Implementations

We've recently hardened the application with the following:
